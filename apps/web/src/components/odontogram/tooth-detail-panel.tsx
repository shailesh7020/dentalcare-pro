...
export function ToothDetailPanel({
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg shadow-md flex flex-col h-full max-h-[720px] overflow-hidden">
      {/* Header */}
      <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex items-start justify-between bg-slate-50/50 dark:bg-slate-800/50">
        <div>
          ...
          <h3 className="text-xs font-semibold text-slate-700 dark:text-slate-300 mt-1">
      ...
      {/* Error Alert if any */}
      {errorMessage && (
        <div className="mx-4 mt-3 p-2.5 bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-800 rounded-md text-xs text-rose-800 dark:text-rose-300 flex items-start gap-2">
      ...
      {/* Navigation Subtabs */}
      <div className="flex border-b border-slate-200 dark:border-slate-800 px-4 bg-white dark:bg-slate-900 text-xs font-medium text-slate-600 dark:text-slate-300">
        <button
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {tooth.surfaces.map((surf) => (
                ...
                >
                  ...
                  {surf.treatment !== "NONE" && (
                    <div className="mt-1 text-[11px] text-teal-800 dark:text-teal-400 font-medium">
            ...
            {selectedSurface && (
              <form
                ...
                className="mt-4 p-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md space-y-2.5"
              >
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-slate-100 rounded px-2 py-1 text-xs"
                    >
        ...
        {activeTab === "history" && (
          <div className="space-y-3">
            ...
            {tooth.history && tooth.history.length > 0 ? (
              <div className="relative pl-5 space-y-3.5 before:absolute before:left-2 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-200 dark:before:bg-slate-700">
                {tooth.history.map((h) => (
                  <div key={h.id} className="relative group">
                    <span className="absolute -left-5 top-1 w-2.5 h-2.5 rounded-full bg-teal-500 ring-4 ring-white dark:ring-slate-900" />
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <p className="text-slate-600 dark:text-slate-300 mt-0.5">{h.description}</p>
                      {h.affected_surfaces && (
                        <span className="inline-block mt-1 px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded text-[10px] font-mono">
              ...
              <div className="text-center py-6 text-slate-400 dark:text-slate-500">
                <Clock size={24} className="mx-auto mb-1 text-slate-300 dark:text-slate-600" />
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              <button
                ...
                disabled={isSubmitting}
                className="w-full flex items-center justify-between p-2.5 bg-emerald-50 dark:bg-emerald-950/30 hover:bg-emerald-100/80 dark:hover:bg-emerald-950/50 border border-emerald-200 dark:border-emerald-800 text-emerald-800 dark:text-emerald-300 rounded-md font-semibold text-xs transition-colors"
              >
              ...
              <button
                ...
                className="w-full flex items-center justify-between p-2.5 bg-rose-50 dark:bg-rose-950/30 hover:bg-rose-100/80 dark:hover:bg-rose-950/50 border border-rose-200 dark:border-rose-800 text-rose-800 dark:text-rose-300 rounded-md font-semibold text-xs transition-colors disabled:opacity-50"
              >
              ...
              <button
                ...
                className="w-full flex items-center justify-between p-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700 text-slate-800 dark:text-slate-200 rounded-md font-semibold text-xs transition-colors disabled:opacity-50"
              >
              ...
              <button
                ...
                disabled={isSubmitting}
                className="w-full flex items-center justify-between p-2.5 bg-amber-50 dark:bg-amber-950/30 hover:bg-amber-100/80 dark:hover:bg-amber-950/50 border border-amber-200 dark:border-amber-800 text-amber-800 dark:text-amber-300 rounded-md font-semibold text-xs transition-colors disabled:opacity-50"
              >
